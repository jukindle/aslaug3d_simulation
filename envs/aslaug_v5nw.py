...
class AslaugEnv(aslaug_base.AslaugBaseEnv):
    ...
    def reset(self, init_state=None, init_setpoint_state=None,
        ...
        if "episode_end_callback" in self.p:
            ...
            self.done_info = None
        if "reward_est_train_callback" in self.p:
            self.p["reward_est_train_callback"](self.init_state, self.cum_rew)
        ...
        if "reward_est_callback" in self.p:
            init_state_candidates = []
            for i in range(20):
                obs_i = self.sample_init_state()
                init_state_candidates.append(obs_i)
            rews = self.p["reward_est_callback"](init_state_candidates)
            obs = init_state_candidates[np.argmin(rews)]
        else:
            obs = self.sample_init_state()
