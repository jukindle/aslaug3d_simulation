...
class AslaugEnv(gym.Env):
    ...
    def __init__(self, folder_name="", gui=False):
        ...
        self.params = {
            "joints": {
                "controllable_joints": [0, 3],
                "joint_names": ['panda_joint{}'.format(i+1) for i in range(7)],
                "init_states": [-np.pi/2, np.pi/2, np.pi/2, -1.75, -np.pi/2, np.pi, np.pi/4],
        ...
        high_lp = np.array((self.n_joints+1)*[1.5, 1.5,
                                              np.pi])
        low_lp = -high_lp
        high_j_p = self.joint_limits[[0, 3], 1]
        low_j_p = self.joint_limits[[0, 3], 0]
        high_j_v = np.array([self.p["joints"]["vel_mag"]]*2)
        low_j_v = -high_j_v
    ...
    def step(self, action_d):
        ...
        # Obtain actions
        mb_actions = np.choose(action_d[0:3], self.actions[:, 0:3])
        joint_actions = np.zeros(self.n_joints)
        joint_actions[[0, 3]] = np.choose(action_d[3:], self.actions[:, 3:])
    ...
    def calculate_observation(self):
        sp_pose_ee = self.get_ee_sp_transform()[[1, 2, 4]]
        link_pose_r = self.get_link_states(self.link_mapping[2:])[:, [0, 1, 5]]
        j_pos, j_vel = self.get_joint_states()
        j_pos = j_pos[[0, 3]]
        j_vel = j_vel[[0, 3]]
    ...
    def reset(self, init_state=None, init_setpoint_state=None,
        ...
        j0 = self.np_random.uniform(self.joint_limits[0, 0], self.joint_limits[0, 1])
        j3 = self.np_random.uniform(self.joint_limits[3, 0], self.joint_limits[3, 1])
        ...
        robot_pos = (-0.5, 0, 0.02)
        ori = self.np_random.uniform(np.pi/4, np.pi/2 + np.pi/4)
        robot_ori = pb.getQuaternionFromEuler([np.pi/2, 0, ori])
    ...
    def calculate_goal_distance(self):
        ...
        eucl_dis = np.linalg.norm(sp_pose_ee[1:3])  # Ignore (z in world)
        eucl_ang = np.linalg.norm(sp_pose_ee[4])
