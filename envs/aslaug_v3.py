...
# Aslaug environment with replay buffer
class AslaugEnv(aslaug_base.AslaugBaseEnv):
    ...
    def reset(self, init_state=None, init_setpoint_state=None,
        ...
        if self.soft_reset:
            obs = self.soft_reset_env()
        else:
            if len(self.replay_buffer) < self.p["replay_buffer"]["length"]:
                if self.init_states is not None:
                    self.init_states["acc_rew"] = self.acc_rew
                    self.replay_buffer.append(self.init_states)

            else:
                val_max = np.inf
                idx_max = 0
                for i in range(len(self.replay_buffer)):
                    acc_rew_i = self.replay_buffer[i]["acc_rew"]
                    if acc_rew_i > val_max:
                        val_max = acc_rew_i
                        idx_max = i

                if self.acc_rew <= val_max:
                    self.init_states["acc_rew"] = self.acc_rew
                    self.replay_buffer[idx_max] = self.init_states

        # Randomly choose a reset method (normal reset or replay buffer)
        if len(self.replay_buffer) > 0 and \
                self.np_random.rand() <= self.p["replay_buffer"]["prob_replay"]:
            # Select min element from replay buffer
            val_min = -np.inf
            idx_min = 0
            for i in range(len(self.replay_buffer)):
                acc_rew_i = self.replay_buffer[i]["acc_rew"]
                if acc_rew_i < val_min:
                    val_min = acc_rew_i
                    idx_min = i
            # idx = self.np_random.randint(len(self.replay_buffer))
            init_states = self.replay_buffer.pop(idx_min)
            obs, self.init_states = self.restore_env(init_states)
        else:
            # Normal reset
            obs, self.init_states = self.hard_reset_env()

        # Randomly forget an entry in the replay buffer
        if len(self.replay_buffer) > 0:
            if self.np_random.rand() <= self.p["replay_buffer"]["prob_forget"]:
                # Select min element from replay buffer
                val_min = -np.inf
                idx_min = 0
                for i in range(len(self.replay_buffer)):
                    ...
                    if acc_rew_i < val_min:
                        ...
                        idx_min = i
                # idx = self.np_random.randint(len(self.replay_buffer))
                self.replay_buffer.pop(idx_min)

        return obs

    def soft_reset_env(self):
    ...
    def hard_reset_env(self):
        ...
        # Randomize object positions in world
        self.possible_sp_pos, poses_dict = self.randomize_environment()

        # Reset robot base velocities
        pb.resetBaseVelocity(self.robotId, [0, 0, 0], [0, 0, 0], self.clientId)

        # Reset robot pose and actuated joint poses
        joint_states = self.p["joints"]["init_states"].copy()
        for i in range(100):
            ...
            pb.resetBasePositionAndOrientation(self.robotId, robot_pos, robot_ori,
                                               self.clientId)
            # Reset arm to 2D locked position
            ...
            for i in range(len(self.actuator_selection)):
                if self.actuator_selection[i]:
                    jp = self.np_random.uniform(self.joint_limits[i, 0],
                                                self.joint_limits[i, 1])
                    pb.resetJointState(self.robotId, self.joint_mapping[i],
                                       jp, 0.0, self.clientId)
                    joint_states[i] = jp
        ...
        # Return observation and initial state
        init_state = {"poses_list": poses_dict, "robot_pos": robot_pos,
                      "robot_ori": robot_ori, "joint_states": joint_states,
                      "sp_pos": sp_pos}
        return obs, init_state

    def restore_env(self, init_states):
        # Reset internal parameters
        self.episode_counter += 1
        self.step_no = 0
        self.integrated_hold_reward = 0.0
        ...
        # Randomize object positions in world
        self.possible_sp_pos, poses_dict = self.randomize_environment(init_states["poses_list"])

        # Reset robot base velocities
        pb.resetBaseVelocity(self.robotId, [0, 0, 0], [0, 0, 0], self.clientId)

        # Reset robot pose
        pb.resetBasePositionAndOrientation(self.robotId,
                                           init_states["robot_pos"],
                                           init_states["robot_ori"],
                                           self.clientId)

        # Reset arm positions
        for i in range(len(self.joint_mapping)):
            pb.resetJointState(self.robotId, self.joint_mapping[i],
                               init_states["joint_states"][i],
                               0.0, self.clientId)

        # Reset setpoint position
        sp_pos = self.reset_setpoint(init_states["sp_pos"])

        # Calculate observation
        obs = self.calculate_observation()
        self.last_yaw = None

        return obs, init_states

    def reset_setpoint(self, init_pose=None):
        if init_pose is None:
            # Spawn random setpoint
            sp_pos = random.sample(self.possible_sp_pos, 1)[0]
        else:
            # Spawn predefined setpoint
            sp_pos = np.array(init_pose)
    ...
    def randomize_environment(self, poses_list=None):
        # Randomize bookcases
        possible_sp_pos = []
        pos = [1.47 / 2 + 7.9, 0, 0]
        ...
        if self.randomized_env:
            if poses_list is None:
                for key in self.random_objects:
                    ...
                    poses_list_out.append({"id": id, "pose2d": pose2d,
                                           "visible": visible})
            else:
                poses_list_out = poses_list
                for key in poses_list:
                    id = key["id"]
                    pose2d = key["pose2d"]
                    visible = key["visible"]
                    if visible:
        ...
        return possible_sp_pos, poses_list_out
