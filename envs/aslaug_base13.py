...
class AslaugBaseEnv(gym.Env):
    ...
    def step(self, action_d):
        ...
        # Clip for actuator limits
        j_pos, _ = self.get_joint_states()
        filt = self.actuator_selection
        filt_high = ~(filt*((self.joint_limits[:, 1] - j_pos) <= 1e-3))
        filt_low = ~(filt*((j_pos - self.joint_limits[:, 0]) <= 1e-3))
        crop_low = filt_low.astype(float)
        crop_high = filt_high.astype(float)
        crop_low[filt_low] = -np.inf
        crop_high[filt_high] = np.inf
        joint_actions = np.clip(joint_actions, crop_low, crop_high)
