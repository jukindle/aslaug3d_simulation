...
class AslaugEnv(aslaug_base.AslaugBaseEnv):
    ...
    def spawn_bookcases(self, n, easy=False):
        ...
        kallax2 = pb.loadURDF(model_path, [1.47 / 2 + 12.0, 0, 0],
                              useFixedBase=True, physicsClientId=self.clientId)
        kallax3 = pb.loadURDF(model_path, [1.47 / 2 + 4.0 + 1.48, 0, 0],
                              useFixedBase=True, physicsClientId=self.clientId)
        kallax4 = pb.loadURDF(model_path, [1.47 / 2 + 12.0 + 1.48, 0, 0],
                              useFixedBase=True, physicsClientId=self.clientId)
        self.bookcaseIds = [kallax1, kallax2, kallax3, kallax4]
    ...
    def randomize_environment(self, force_new_env=False):
        ...
        possible_sp_pos += self.move_bookcase(self.bookcaseIds[1], pos,
                                              sp_layers=layers)
        pos = [1.47 / 2 + 4 + 1.48 + self.np_random.uniform(-0.02, 0.04), 0, 0]
        possible_sp_pos += self.move_bookcase(self.bookcaseIds[2], pos,
                                              sp_layers=layers)
        pos = [1.47 / 2 + 12 + 1.48 + self.np_random.uniform(-0.02, 0.04), 0, 0]
        possible_sp_pos += self.move_bookcase(self.bookcaseIds[3], pos,
                                              sp_layers=layers)
