...
class AslaugEnv(aslaug_base.AslaugBaseEnv):
    ...
    def __init__(self, folder_name="", gui=False, free_cam=False,
        ...
        port = self.p["discriminator"]["port"]
        self.discr_com = DiscriminatorCommunication(port)
    ...
    def reset(self, init_state=None, init_setpoint_state=None,
        ...
        if self.init_state is not None:
            self.discr_com.add_training_sample(self.init_state, self.cum_rew)
        ...
        init_state_candidates_obs = []
        init_state_candidates = []

        for i in range(30):
            obs_i, states_i = self.sample_init_state()
            init_state_candidates_obs.append(obs_i)
            init_state_candidates.append(states_i)
        rews = self.discr_com.predict(np.array(init_state_candidates_obs))
        idx_min = np.argmin(rews)
        obs = init_state_candidates_obs[idx_min]

        (robot_pos, robot_ori, j0, j3) = init_state_candidates[idx_min]
    ...
    def sample_init_state(self):
        ...
        return self.calculate_observation(), (robot_pos, robot_ori, j0, j3)
...
class DiscriminatorCommunication:
    ...
    def send_msg(self, msg):
        ...
        while not success:
            try:
                conn = ClientWithTimeout(self.address, authkey=b"tuemecrifuieR11NAF10", timeout=1.0)
                conn.send(msg)
                response = conn.recv()
                conn.close()
                success = True
