...
class AslaugEnv(aslaug_base.AslaugBaseEnv):
    ...
    def reset(self, init_state=None, init_setpoint_state=None,
        ...
        if self.soft_reset:
            ...
            success = self.generate_occmap_path()
            if not success:
                obs = self.reset()
        ...
        while collides:
            ...
            # HACK: Try to prevent "Verlernung der Skills"
            srx = 3.0 if self.np_random.uniform() <= 0.5 else self.p["world"]["spawn_range_x"]
            x_min = np.max((0.0, sp_pos[0] - srx))
            x_max = np.min((cl, sp_pos[0] + srx))
    ...
    def generate_occmap_path(self):
        ...
        self.path, path_idx, suc = self.occmap.generate_path(pos, n_its=25000)
...
class OccupancyMap:
    ...
    def generate_path(self, pos, n_its=5000):
        ...
        path, path_idx, success = self.find_path(harm, pos)
        #self.visualize_path(path_idx, harm)
        return path, path_idx, success
    ...
    def find_path(self, harm, pos):
        ...
        for i in range(int(self.res*(self.x_u-self.x_l)*10)):
            ...
            if self.idx_sp[0] == x and self.idx_sp[1] == y:
                break
        success = True
        goal_vec = (np.array(path[-1])-np.array(self.pos_sp[0:2]))
        if np.linalg.norm(goal_vec) > 2/self.res:
            print("ERROR WHEN FINDING HPT PATH. Distance was {}".format(np.linalg.norm(goal_vec)))
            success = False
        path[-1] = self.pos_sp[0:2]
        return path, path_px, success
