...
class AslaugPolicy(RecurrentActorCriticPolicy):
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch,
                 n_lstm=64, reuse=False, **kwargs):
        ...
        with tf.variable_scope("model/scan_block", reuse=reuse):
            ...
            s1_7 = tf.layers.Dense(256, activation=lrelu, name="s1_7")(s1_6)
            s1_out = tf.layers.Dense(128, activation=lrelu, name="s1_out")(s1_7)
            ...
            s2_7 = tf.layers.Dense(256, activation=lrelu, name="s2_7")(s2_6)
            s2_out = tf.layers.Dense(128, activation=lrelu, name="s2_out")(s2_7)
        ...
        with tf.variable_scope("model/memory_block", reuse=reuse):
            m_0 = batch_to_seq(c_out, n_env, n_steps)
            masks = batch_to_seq(self.dones_ph, n_env, n_steps)
            m_1, self.snew = lstm(m_0, masks, self.states_ph, 'lstm1', n_hidden=n_lstm)
            m_out = seq_to_batch(m_1)
    ...
    def step(self, obs, state=None, mask=None, deterministic=False):
        if deterministic:
            return self.sess.run([self.deterministic_action,
                                  self.value_flat,
                                  self.snew,
                                  self.neglogp],
                                 {self.obs_ph: obs,
