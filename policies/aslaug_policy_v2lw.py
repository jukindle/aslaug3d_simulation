...
class AslaugPolicy(ActorCriticPolicy):
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch,
        ...
        with tf.variable_scope("model/scan_block", reuse=reuse):
            ...
            s1_2 = tf.layers.Conv1D(8, 7, activation=lrelu, name="s1_2")(s1_1)
            s1_3 = tf.layers.MaxPooling1D(10, 8, name="s1_3")(s1_2)
            s1_4 = tf.layers.Conv1D(8, 7, activation=lrelu, name="s1_4")(s1_3)
            s1_5 = tf.layers.Conv1D(4, 5, activation=lrelu, name="s1_5")(s1_4)
            s1_6 = tf.layers.Flatten(name="s1_6")(s1_5)
            s1_7 = tf.layers.Dense(64, activation=lrelu, name="s1_7")(s1_6)
            s1_out = tf.layers.Dense(32, activation=lrelu, name="s1_out")(s1_7)

            s2_0 = tf.expand_dims(in_sc2, -1)
            s2_1 = tf.layers.Conv1D(4, 11, activation=lrelu, name="s2_1")(s2_0)
            s2_2 = tf.layers.Conv1D(8, 7, activation=lrelu, name="s2_2")(s2_1)
            s2_3 = tf.layers.MaxPooling1D(10, 8, name="s2_3")(s2_2)
            s2_4 = tf.layers.Conv1D(8, 7, activation=lrelu, name="s2_4")(s2_3)
            s2_5 = tf.layers.Conv1D(4, 5, activation=lrelu, name="s2_5")(s2_4)
            s2_6 = tf.layers.Flatten(name="s2_6")(s2_5)
            s2_7 = tf.layers.Dense(64, activation=lrelu, name="s2_7")(s2_6)
            s2_out = tf.layers.Dense(32, activation=lrelu, name="s2_out")(s2_7)

            sc_0 = tf.keras.layers.Concatenate(name="sc_0")([s1_out, s2_out])
            sc_1 = tf.layers.Dense(64, activation=lrelu, name="sc_1")(sc_0)
            sc_2 = tf.layers.Dense(32, activation=lrelu, name="sc_2")(sc_1)
            sc_out = tf.layers.Dense(16, activation=lrelu, name="sc_out")(sc_2)

        with tf.variable_scope("model/attraction_block", reuse=reuse):
            at_0 = tf.keras.layers.Concatenate(name="at_0")([in_sp, in_mb,
                                                             in_jv])
            at_1 = tf.layers.Dense(64, activation=lrelu, name="at_1")(at_0)
            at_2 = tf.layers.Dense(32, activation=lrelu, name="at_2")(at_1)
            at_out = tf.layers.Dense(16, activation=lrelu, name="at_out")(at_2)

        with tf.variable_scope("model/repulsion_block", reuse=reuse):
            rp_0 = tf.keras.layers.Concatenate(name="rp_0")([in_lp, sc_out,
                                                             in_jv, in_mb])
            rp_1 = tf.layers.Dense(64, activation=lrelu, name="rp_1")(rp_0)
            rp_2 = tf.layers.Dense(32, activation=lrelu, name="rp_2")(rp_1)
            rp_out = tf.layers.Dense(16, activation=lrelu, name="rp_out")(rp_2)

        with tf.variable_scope("model/combination_block", reuse=reuse):
            c_0 = tf.keras.layers.Concatenate(name="c_0")([at_out, rp_out,
                                                           in_jp])
            c_1 = tf.layers.Dense(64, activation=lrelu, name="c_1")(c_0)
            c_2 = tf.layers.Dense(32, activation=lrelu, name="c_2")(c_1)
            c_out = tf.layers.Dense(16, activation=lrelu, name="c_out")(c_2)

        with tf.variable_scope("model/actor_critic_block", reuse=reuse):
            vf_0 = tf.layers.Dense(32, activation=lrelu,
                                   name="vf_0")(c_out)
            vf_latent = tf.layers.Dense(16, activation=lrelu,
                                        name="vf_latent")(vf_0)
            pi_0 = tf.layers.Dense(32, activation=lrelu,
                                   name="vf_0")(c_out)
