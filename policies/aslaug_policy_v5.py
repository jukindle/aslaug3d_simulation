...
class AslaugPolicy(ActorCriticPolicy):
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch,
        ...
        with tf.variable_scope("model/scan_block", reuse=reuse):
            ...
            sl_1 = tf.layers.Conv1D(16, 5, activation=lrelu, name="sc_1", padding='same')
            sl_2 = tf.layers.MaxPooling1D(7, 7, name="sc_2", padding='same')
            sl_3 = tf.layers.Conv1D(16, 3, activation=lrelu, name="sc_3", padding='same')
            sl_4 = tf.layers.Conv1D(16, 3, name="sc_4", padding='same')
            sl_5 = tf.layers.BatchNormalization(name="sc_5")
            sl_6 = tf.nn.leaky_relu
            sl_7 = tf.layers.Conv1D(16, 3, activation=lrelu, name="sc_7", padding='same')
            sl_8 = tf.layers.BatchNormalization(name="sc_8")
            sl_9 = tf.layers.Conv1D(16, 3, name="sc_9", padding='same')
            sl_10 = tf.layers.BatchNormalization(name="sc_10")
            sl_11 = tf.nn.leaky_relu
            sl_12 = tf.layers.AveragePooling1D(9, 9, name='sc_12')
            sl_out = tf.layers.Flatten()
            ...
            s2_9 = sl_9(s2_8)
            s2_10 = sl_10(s2_9)
            s2_11 = sl_11(s2_10 + s2_5)
            s2_12 = sl_12(s2_11)
            s2_out = sl_out(s2_12)

            sc_0 = tf.keras.layers.Concatenate(name="sc_0")([s1_out, s2_out])
            sc_1 = tf.layers.Dense(512, activation=lrelu, name="sc_1")(sc_0)
            sc_2 = tf.layers.Dense(256, activation=lrelu, name="sc_2")(sc_1)
